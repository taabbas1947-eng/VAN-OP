...
## Pass forty-eight — 25h: How are we doing (the Dashboard, designed with Tahir) (24 Sep, NOT pushed)

**Rulings (24 Sep):** COO, CFO, Plant Manager only; 4 questions — selling to budget, delivering on time, where the plant is stuck, is quality holding; this month with the FY beside it; one page, 4 blocks, no tabs.
**Built:** `HOW_ROLES`, `howMay/howPeriod/howIn/howSales/howOnTime/howStuck/howQuality/howM/howTile/howBlock/screenHow`. Sales: saleRows (net, truck out) month and FY, FY budget = Σ salesTargets (same as Sales & Budget), month target from monthlyTargetTotals, open = Σ saleOpenValue, share of FY gone → on/behind pace, by channel (budgetByChannel). On time: lines delivered in full with deliveredDate in period, on time if ≤ committed (else promised); past promise now = lineOverdue (not closed short), value = open value. Stuck: actionItems + actTiming, by acStageOf, top 5 oldest with the role holders' names. Quality: COAs approved in period, first-time = no rejected/reworks/deviation/supersedes, copies of a tested lot (repOf) not counted; unfit now; trucks failed inspection (psiRegisterRows); late lab tests. Each block opens Sales & Budget / Orders behind promise / Plant / Lab certificates.
Wiring: screenDash → screenHow; old tab code kept as screenDashOld, reachable only as Sales & Budget (screenBudget → screenDashOld, no tab bar, no narrative banner). NAV_GROUPS gains 'How are we doing' (renderTopNav shows it only to howMay()). scr 'dash' renamed, owners → Plant Manager, CFO, COO. Top bar hidden on dash. The Firefighter, Centers, Production, Quality, Supply tabs are no longer reachable (code kept).
Also (Tahir: "where will one check if the pre-shipment report needs to be sent? e.g. LCI"): the setting is the customer record (Back Office → Customers → the customer → "Send the pre-shipment report with the DC"); now also shown on the truck card and every truck job for that customer. Truck card in Loading: Saad sees Review, the Plant Manager "Approve DC & release", others see who it is waiting on.
Tests: how.test.js 24; shell, rolemodel, warehousesplit, firefighter updated. Suite 0 failed. Browser: COO desktop, Plant Manager at 390 px (no sideways scroll), a Lab Rep told where his work is; 0 errors.
...
## Pass fifty-one — 25k: Sales & Budget rebuilt; All actions, Firefighter, Centers retired (24 Sep, NOT pushed)

Tahir: "retire them"; Sales & Budget "very badly designed"; "why no budget for this year?"; "the date selector doesn't work".
Found: the budget is set by channel (channelTargets 2026-27 = PKR 1,042,700,000) but the headline and How are we doing read the client targets (salesTargets, empty on live) → "PKR 0 target, 0%, gap 0" and "No budget is set". The period box was the old top bar's; the screen never read it.
Built: `sbPer/sbRange/sbData/sbToggle` + new `budgetHtml` (old one replaced): own period chips (FY to date, this month, last month, any of the last 12 months); budget = Σ channel totals (client targets only if no channel total); tiles: budget (or month target), sold in period with % and year gone, still to reach, open orders and coverage; channels with customers underneath (tap), products sold in the period, monthly pace for FY. screenBudget renders it directly (no old tabs); top bar hidden. howSales uses the channel totals too. Retired: dashFireHtml, dashCentersHtml, fireLight, screenApprovals (setScreen('approvals') → Today; route map approvals → screenToday; search skips it); fireLists kept (Plant uses it).
Live check of the new numbers (read-only, the functions evaluated in Tahir's tab): FY to date sold 152,766,223 (budget 1,042,700,000); Sep 65,660,522; Aug 10,382,449; open 170,412,120 — equal to the old screen's sold and open.
Open: Tahir's White Label client budget (PKR m): Syngenta 424, Rudolf 135, Maxim 128, LCI 71, UDPL 42, UPL 32, BKK 19, FMC 16, Kisan 6 = 873 vs channel 872.7. UPL has no customer record; BKK has 2 records (Distributor and White-label) under one name, and budgetChannelOf puts "BKK" in Distributor; Kisan has already sold 7.28 m against 6. Awaiting his rulings before allocating.


## Pass fifty-two — 25l and 25m: budget per channel; the independent review and its critical fixes (25 Sep, NOT pushed)

**25l:** a customer in 2 channels (BKK: Distributor and White Label) gets a target per channel, keyed "NAME @ CHANNEL" (`BUDGET_SEP`, `budgetSplitKey`, `budgetChannelsOfName`, `budgetTargetOf`; budgetChannelOf and budgetClients understand it; sbData reads it).
**Tahir's White Label client budget (PKR m), reconciled:** Syngenta 424, Rudolf 135, Maxim 128, LCI 71, UDPL 42, UPL 32 (= ARYSTA LIFE SCIENCES (PVT) LTD, "UPL is same as Arysta"), BKK 19 (White Label only), FMC 16 (FMC Pakistan), Kisan 6 = 873.0; the channel total is raised from 872.7 to 873.0 (ruled). Not yet written to live at the time of this entry.
**The independent review (Tahir: "deploy an independent agent who validates the purpose is achieved ... user adoption, easy, clarity, bug free, organisation backbone").** 3 agents on a local copy: UX/adoption, bugs/rules, backbone/board value. Their full reports are summarised in the review doc (see the next pass). Critical/high bugs fixed in 25m:
- Dates (UTC+5): `localDateOf`, `calDays`, `ymdUTC`; evDaysLate, actTiming and every `Math.round((TODAY-…)/86400000)` now calendar days; periodWindow rebuilt on plain dates (This month started on the 31st; today was "1 day late" from the morning).
- The same stock on 2 trucks: reconcileLotShipKgV1 counts material on live not-yet-counted trucks (`onTruck`); dedupeShipmentsV1 looks only at counted trucks.
- Trucks bypassing the gate: saveDispatch (the Today "Plan the truck" path) now creates stage 'truck_planned'; approveDC refuses a staged truck (routes a loading one to approveRelease) and any truck without a passed inspection; saveShipEdit refuses an unreleased truck and any quantity change, and records a correction; dfSubmitShipment is COO only and capped at packed/ordered minus dispatched; dispQASubmit fail subtracts only counted kg and releases the lots.
- COA status order enforced in coaApprove (reviewed) and coaReview (analysed).
- Money: printPO and its Shipments button for mayMoney() only; screenBudget for mayMoney() only.
- Crashes: submitDivert (divertForm → divForm); `var REG={}`; the nav groupOf renamed navGroupOf (it overwrote the product-group groupOf, so lead times used the default rate).
- Dealers on New order: any status except pending/inactive (they were 'Confirmed' and never offered).
- XSS on What to make: brand, base, PO #, client escaped.
- Delivery ladder as ruled: Saad from day 1, Plant Manager from day 2.
...
## Pass fifty-four — 25o: free samples (FOC) get their own path (25 Sep, NOT pushed)

Tahir (25 Sep, before sleeping): a system for Irfan, Muhammad Ali and others who send FOC samples; they ask in the system or a Finance person asks for them; COO approval before any sample goes out; the whole documentation chain except production; see who gave which sample to whom and why, and any feedback; remove FOC from New order and give samples a dedicated path and chain. "Suggest how" — he was asleep, so built on these ASSUMPTIONS (to confirm):
1. Who asks: KAM for himself; Finance, Finance Desk Officer and the CFO on someone's behalf (the person can also be someone not in O2S, typed). Both names kept.
2. COO alone approves (sample.approve, hard, roles []). A "no" needs a reason.
3. Chain: request → COO → Warehouse / Supply Chain Officer issue it naming the lot (a packed lot of that product, showing batch, expiry and COA; or "somewhere else" with a typed batch no.) → QA Inspector checks 5 points (not the person who issued it) → Warehouse issues sample DC (own series SDC-0001) and a gate pass from the SAME series as trucks (nextGatePassNo reads samples too) and prints 3 copies → the person confirms arrival → feedback (result, note, PO it led to), asked 14 days after arrival (SMP_FEEDBACK_DAYS).
4. No Saad review / Plant Manager release on samples (the COO has approved up front). Say so to Tahir.
5. STOCK: the lot is recorded for traceability ONLY — no kg is taken off any lot, batch, order or truck. Where samples physically come from (packed stock of an order, batch bulk, retained stock, a sample stock) needs Tahir's ruling before any stock arithmetic is added.
6. New order: PO type is Commercial only, with a link to Samples. Existing FOC orders keep working (o.foc untouched everywhere).
Built: state.samples[] (merge3 merges by id); `SMP_*` constants; smpList/smpById/smpKg/smpNextNo/smpStamp/smpMine/smpOwner/smpSince/smpFeedbackDue/smpLog/smpUnit/smpProducts/smpCustomers/smpPeople/smpJobs; smpNew/smpRenderForm/smpSubmit; smpOpen/smpRenderOpen/smpDecide/smpIssue/smpCheck/smpDispatch/smpArrived/smpFeedback/smpCancel; smpPrint; screenSamples (tiles, filters, register, this FY by person and by client). Rights sample.request/approve/issue/check (legacy hard; sample.check filed under Quality). SCREENS 'samples', NAV 'Samples' (smpMayView), render map, topbar hidden. Today: actionItems pushes smpJobs; actTiming reads it.smp; TD_LABEL / TD_NEXT / SIGN_JOBS (KAM now has jobs on his Guide); tdStarts "+ Ask for a free sample". Guide rule "Free samples". What's new 25o.
Tests: samples.test.js 32; authmodel (NEW_RIGHTS, WANT, Quality dept, SC filter), rolemodel (legacy names 11), dispatchgrants (Warehouse also sample.issue), shell (6 nav groups; sign-off jobs). Suite 10,481 passed, 0 failed. Browser (port 3940): KAM asks from Today; Finance on behalf; COO approves one and refuses one (reason required); warehouse issues; gate pass refused before QA; QA passes; DC SDC-0001 + GP-0002 printed; KAM confirms and records feedback; orders, lots, shipments, sales, open value and What to make counts identical before and after; 390 px no sideways scroll; Production does not see Samples; 0 errors.
**25o, round-3 review (1 adversarial agent) and fixes, same build:** the chain held against skipped steps, wrong roles and repeats; 25n regressions all held; 0 page errors across 12 users. Fixed:
- 2 people issuing at once got the same gate pass and SDC number. `smpDispatch` is now async: it reads the server's latest copy (`/api/state`) and takes the higher of both (`smpMaxNos`), refuses if the server already shows the sample sent, and saves at once. Truck gate passes (`issueGatePass`) still count from the local copy — the full answer for both is a server-side counter (OPEN, needs a server change).
- `acStageOf`: sample jobs are 'Free samples', not Production, on How are we doing.
- Feedback only by the person who gave the sample, who entered it, or the COO.
- The gate pass date cannot be before the QA check or the approval.
- The COO approving a sample he asked for or entered must write a note; `approval.self` is shown on the register.
- QA cannot pass the COA line on a batch with no approved COA unless he types the COA no. he has seen (kept with his name, `coaTypedBy`).
...
## Pass fifty-six — 26a: see before you sign, close only what you tick, send back with a note, removals stick (26 Sep, NOT pushed)

Tahir's list from the floor (26 Sep): AP26012 made 1,010 of 2,710, no Close batch; the Production Manager could only ask to close a whole PO; a few bags filled from stock O2S does not track; Fahim refused close requests that came back; Fahim and Saad could not see the DC or gate pass before approving; the QCM needs to send reports back with a note; 2 duplicate lots (AP26012-L2, VMG10419-L2) the lab could not close.
Rulings: truck sign-offs get preview + Confirm + Send back (review and release only; to whoever issued the gate pass); a refusal needs a reason and goes back to the asker; small bags from stock = close short with the new reason "Supplied from stock outside O2S" (not counted against us); Close batch on every open batch card, PM's Today carries close jobs, tick products in the close sheet (none ticked by default), fix made-so-far on multi batches; lab: QCM sends back to AQCM, AQCM to the analyst who drafted it (parameter-wise testing stays off), full chain re-signs, approved = re-issue (Rev); fix the sync merge so removals stick (approved); Claude clears the 2 leftover lots on live AFTER the push.
Built (o2s/o2s.html): rejectShortClose keeps the request with refusedBy/At/Why (never deletes), openShortCloseRefuse, openRefusal / refusalSeen, scArchive to shortCloseHistory, scAsker, Today job 'Refused'; openClosePO/renderClosePO/submitClosePO with tick boxes (cpPicked/cpTick/cpTickAll), card button "Ask to close products…"; SHORTCLOSE_REASONS + outside_o2s; Close batch on the Running card and list row (edClose, output > 0); TD_RIGHT 'Close batch'/'Reopened — check first' → batch.close; runBatchesFor includes multi batches; multiTrueUp + multiTrueUpIfPlaced (only closed AND fully placed; checked on live: MAXKL26005 would otherwise have lost 22868's 1,000), hooked into doCloseBatch, doPack, saveReconcile, al.credited recorded at shift log; truck sheet openTruckSign / tsSheetHTML / sendBackTruck / openTruckSentBack / truckFixed, TS_STEP, Today acts + Shipments card routed through it, 'Sent back' job, review clock restarts at the fix; dl.facts CSS; merge3 honours removals (3-way, reference guard for the no-baseline fallback); lotLeftoverDup + Remove of a leftover duplicate record (produced and orders unchanged), banner + tag on the Lots tab; lab coaSendBack / coaSendBackOpen / coaReturnBanner / coaOpenReturn / coaCloseReturn, returns[] + returnOpen, Today 'Correct COA' (to the drafter by name, timed from the return, escalates to QCM after 1 day) and 'Review COA again'; Reject buttons replaced. Guide (How, Rules, job cards), TD_LABEL/TD_NEXT, changelog 26a, BUILD_ID 2026-09-26a.
Tests: new o2s/tests/fixes26a.test.js (76); shortcloseactions, certremove §9 (now asserts the fix), firstsave BUG 5 (fixed), shell, how, guide updated. Suite 10,611 passed, 0 failed. Browser (disposable copy in the cloud, file mode): release sheet, send back, fix, refusal, asker's Today, tick sheet, Close batch on running card; 0 errors. Live read-only checks: merge3 on live state is identity, removals stick both ways, new local records kept, fallback identity.

**After Tahir pushes and the team refreshes:**
- Claude clears AP26012-L2 and VMG10419-L2 on live (Tahir's ruling): lot record + duplicate shift entry + duplicate log rows, produced unchanged; show before/after.
- Fahim's 14 items: the refused ones (Grain Set, Max Amino, RUD2682401) come back once more; refuse again with a reason and they stay refused.
- AP26012: Ali Raza / Jawad (or Abdul Majid) closes it from its card with the reason for 1,700 short; COBO V-Ammonium Phosphate then reads 300 made, 1,700 to make, unless Fahim approves Saad's close.

**Open, needing Tahir:**
1. Back-path audit (26 Sep): Acknowledge PO, Approve PR, Approve customer are one tap with nothing shown and no refuse/send back; Reject DC voids with no reason and tells nobody; sample not approved puts nothing on the asker's Today; Pack QA Correct/Re-inspect notes optional. Asked which to build.
2. Batch report vs lot reports (Tahir, 26 Sep): a 50-ton batch in 8 lots/8 COAs, customer wants one batch report after lot 1. To design after the push.
3. Earlier list (resume section above) still stands: server-side gate pass/DC numbering, server-side rights, COO running a truck alone, old screens in the new skin, board write-up.


## Pass fifty-seven — 26a continued: every approval has a way back (26 Sep)

Tahir (26 Sep): Saad reported the same as Fahim (Review showed nothing); "check this problem across other functions, and check clearly that acceptance, rejection, approval have a back path wired correctly." Audit of every Today job found 4 more; Tahir chose to build all. Batch report after lot 1: Tahir said leave it, build nothing.
Built: Acknowledge PO opens the PO (openAckSheet) with Send back to whoever entered it (ord.enteredBy from now on, else by name, else Finance) → 'PO sent back' job, Correct the order (if allowed) + Fixed → back to Supply Chain; ackOrder refuses while it is back. Approve PR opens the request (openPRSheet, both line and state.prs PRs) with Refuse (reason) → 'PR refused' job to Supply Chain: ask again (with what changed) or drop; refused PRs never absorb new ones or count as open for receiving. Approve customer opens the record (openCustSheet) with Send back → 'Customer sent back' to whoever entered it (rec.createdBy from now on); custSave keeps createdBy/sentBack on edit; approveCustomer refuses while it is back. Reject DC needs a reason (openRejectDC), keeps it on the rows, and Supply Chain gets 'DC rejected' on Today until read (state.dcRejects). A sample not approved goes on the asker's Today until read (smpRejectSeen). Pack QA Correct / Re-inspect notes required. Lab cover, leave and lot-tested-for-batch were already live since 25p (checked on live: no switch; labFlow null = parameter-wise off, as Tahir wants). Rule "See it before you sign it; every no has a way back" in the Guide; changelog 26a extended.
Tests: fixes26a now 103; customerapproval updated. Suite 10,645 passed, 0 failed. Browser (disposable copy): ack sheet, PR sheet + refusal job, COA send back QCM → AQCM with the banner and "Review COA again" job; 0 errors.


## Pass fifty-eight — 26a continued: batch leftovers, Sulfur Coated Urea → Nitro Sulfur, Production Manager forms (26 Sep)

Tahir asked whether the Sulfur Coated Urea / Nitro Sulfur path and each batch's loss, wastage, movement and call-back are configured. Found on live (read only): the rule SCU → Nitro Sulfur is set and the code path exists (reconcile → loss / by-product pool / divert / rework → call back), but it had NEVER been used: 0 reconciles, 0 pools; 57 batches held 147,374 Kg unaccounted (6 closed, 24,412 Kg; SCU 28 batches, 11,025 Kg, 25 to 875 each). Cause: Reconcile appeared only on a closed batch with nothing lab-cleared left, so every cleared leftover sat in Ready to pack with only Pack. Tahir then: "ready to pack should have the reconciliation or movement of stock option we built, you dropped it in the new design; he can't close batch or redirect material" and "don't invent anything except what is already in the logic; ask if you need process clarity".
Rulings: a Packing finished button (Production Manager only) opening the existing reconcile sheet, plus "keep as bulk stock"; SCU pre-filled as by-product to Nitro Sulfur; the backlog goes to Abdul Majid as a list to mark.
Built: Ready to pack cards and the passport (closed batches included) offer "Packing finished · reconcile / move" (packing.reconcile) and Close batch on open batches; reconcile sheet gains Keep as bulk stock (not disposed, b.stockKept); SCU pre-fill = the whole leftover; a second reconcile ADDS loss (b.lossKg, packReconcile cumulative, reconHistory) — it used to overwrite; submitCallBp draws the pool's sources FIFO and records fromSources on the new Nitro Sulfur batch. Log shift output, Close batch and the reconcile sheet restyled (prodSkin/prodTiles: same fields and handlers, shell buttons, tiles). Guide How (Produced) and changelog updated.
...
## Pass sixty — 26b: the Guide covers every change of the day (26 Sep)

Tahir: "update the guides for all changes". Updated in o2s/o2s.html: My job — a "New on 26 Sep" note per role (Production Manager, Production, Plant Manager, Supply Chain, Warehouse, Supply Chain Officer, CFO, Finance, Finance Desk Officer, QCM, AQCM, Lab Rep); How the app works — Materials step (PR refuse, PO send back); Reference steps 3 to 8 (acknowledge sheet and send back, PR refuse, close batch short, Packing finished / reconcile / move incl. Sulfur Coated Urea → Nitro Sulfur pool → either product, lab send back chain, QA correction note, truck sheet and send back, DC rejection reason); Reference roles (Supply Chain, CFO, Production Manager, Plant Manager, Lab Rep, AQCM, QCM); Back Office customers (CFO send back). The Rules, job cards and TD_LABEL were done earlier in the day. Changelog 26b line. guide.test.js 39. Suite 10,670 passed, 0 failed. NOT pushed.


## Pass sixty-one — 26c: a closed batch stays closed (sync baseline fix) + Plant screen opens the sheet (26 Sep)

Tahir: "Batch number AP26012 KEPT COMING BACK AFTER CLOSURE BY PM". Live (read only, rev 9929): 27 batches status 'open' with closedDate 2026-09-26 and closedBy Abdul Majid — AP26012 (varianceKg 1700 kept) and 26 Sulfur Coated Urea bulk batches VU26163–VU26190 (closedBulk, reconcile to the Nitro Sulfur pool saved). The "closed" log rows were missing.
Cause: saveNow's 409 path and startSync's auto-refresh set the baseline with _snapBase() AFTER the merge, i.e. from the merged LOCAL state holding our unsent change. A second conflict then saw local == base for status and took the server's older 'open'; new keys (closedDate...) survived because the server had none; log rows read as "removed by them" (the 26a removal rule — that part made worse by 26a). Existed before 26a for the status leaf.
Fix: _srvCopy(j.data) taken before the merge becomes _baseSnapshot in both paths; after 5 straight conflicts the save retries in 3 s instead of stopping with the change unsent.
Also: Plant screen (fireLists) buttons no longer sign in one tap — issueGatePass / approveRelease / approveDC → openTruckSign(id, step); approveRMPR → openPRSheet('line',...) (refused PRs excluded).
Tests: new fixes26c.test.js (20; drives the real saveNow against a fake server; fails 2 on the old code). Suite 10,690 passed, 0 failed; node --check clean. Browser (disposable copy, two tabs, real server): old 26b → status "open", closedBy kept (the live fingerprint); 26c → "closed", log row kept. 0 page errors.
Guide: changelog 2026-09-26c, BUILD_ID 2026-09-26c; Rules "See it before you sign it" and Plant Manager's New-on-26-Sep note mention the Plant screen.
Rulings: Abdul Majid re-closes the 27 batches himself AFTER 26c is live (not Claude). Next: variance note to the Plant Manager, truck failing inspection, then audit items 5–8 with a question each first.
Audit (22 decision points, agent + 2 verified by hand): open gaps — varianceNotify written, never shown (8172); truck QA fail voids with no reason/no job (dispQASubmit); pending customer made Active via Reactivate without the CFO (custToggleStatus); delivery has no refused/short/damaged outcome; sample review/release no send back; UNFIT rework loses results (coaRework); packed-lot QA hold job goes to the inspector; Rejected COAs report reads coa.rejected (always 0); RM Check re-run overwrites rmPR history.
Scratch: o2s/tests/.old26b.html.tmp (old build copy for the check) is excluded in .git/info/exclude; delete when convenient.
Pass 61 continued (same day, BUILD_ID 2026-09-26d — 26c was already pushed, so the new build id makes open tabs refresh): Tahir's rulings — (1) Yield short: the Plant Manager reads and marks Seen, no send back; (2) a truck failing pre-shipment inspection: Supply Chain re-plans.
Built: yieldShortJobs / openYieldShort / yieldShortSeen (Plant Manager or COO; keyed on status closed + varianceKg > 0.5 + not acked, so a close whose varianceNotify was lost to the sync bug still shows — live: HG26025, HG26027, and AP26012 once re-closed). dispQASubmit: Remarks required (≥ 5) on a fail, checked before anything changes; dcRejectNote(rows, why, 'qa') → Supply Chain job 'Truck failed' (same record list as DC rejected; openDcRejected titles it). TD_LABEL, TD_NEXT, acStageOf updated. Guide: changelog 26c items, rule sentence, My job notes for Plant Manager, Supply Chain, QA Inspector.
Tests: fixes26c 36. Suite 10,706 passed, 0 failed; node --check clean. Browser (disposable copy): Yield short sheet renders (RUBS26003); a failed truck without Remarks is blocked, with Remarks it is voided and "Truck failed" opens with the reason; 0 page errors.
Still open from the audit, each needs Tahir's process answer first: delivery refused/short/damaged; sample review/release send back; UNFIT rework keeps results + tells Production; packed-lot QA hold job owner; plus customer "Reactivate" bypass, Rejected COAs report, RM Check re-run overwriting PR history.
Pass 61 continued (26d): Tahir's rulings on the rest of the audit — delivery: record only; free samples: leave as is; UNFIT rework: keep results, tell Production; QA hold: the Production Manager.
Built: coaRework asks why (≥ 5), keeps the failed COA in h.coaHistory with reworkWhy/By/At, keeps the count on the lot (h.reworks), opens h.rework; Today 'Rework lot' to Production (TD_RIGHT production.enter) and NO lab job until reworkDone (note ≥ 5) — then the normal Lab QC job. QA-held lot job now role 'Production Manager' with the hold reason; clearQaHold allows the Production Manager. Confirm delivery: Condition on arrival (DELIV_COND ok/refused/short/damaged) + Kg/L + note (≥ 5) → s.deliveryIssue, in the log and on the delivered shipment card; delivered quantities unchanged. custToggleStatus refuses a 'Pending approval' customer (R9 bypass closed).
Tests: fixes26c 54. Suite 10,729 passed, 0 failed; node --check clean. Browser (disposable copy): delivery with 'damaged' blocked without Kg, recorded with 50 Kg + note; rework → Production job with reason, no lab job meanwhile, history 1; 0 page errors.
