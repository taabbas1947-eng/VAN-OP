...
database (`van_platform` on his XAMPP) until that work is finished and checked.
Only then does Tahir apply the same migration to production by hand, at a
moment of his choosing. So: never treat a migration as incomplete because
production has not had it yet, never press for the production run as the next
