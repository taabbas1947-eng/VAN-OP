...
const FN = ['smpList','smpById','smpKg','smpNextNo','smpStamp','smpMine','smpOwner','smpSince','smpFeedbackDue','smpLog','smpUnit','smpProducts','smpPeople','smpJobs','smpSubmit','smpDecide','smpIssue','smpCheck','smpDispatch','smpArrived','smpFeedback','smpCancel','smpCoaOf','nextGatePassNo'];
...
vm.runInContext(`smpAct={id:'${x.id}',carrier:'TCS',date:'2026-09-25'}`, b); b.smpDispatch(); eq('no gate pass before QA', x.status, 'issued');
...
as('Warehouse', 'shoaib'); vm.runInContext(`smpAct={id:'${x.id}',carrier:'TCS',tracking:'T1',date:'2026-09-25'}`, b); b.smpDispatch();
...
vm.runInContext(`smpAct={id:'${x.id}',result:'Positive, an order is coming',note:'good',po:'PO-1'}`, b); b.smpFeedback();
