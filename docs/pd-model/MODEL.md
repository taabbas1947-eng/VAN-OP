...
- **Problem** — a real field/market pain. Few, long-lived. *P fixation in our soils.*
...
## 5. What a person actually sees — TWO screens
1. **"What came in"** — the single intake door. One minute, no need to classify.
2. **"What I owe"** — my assigned questions, who waits on each, by when; late shows
   as late. The only scoreboard.
Everything else (claims, grades, library, the full nine) is back-of-house.
...
## 7. Decisions still OPEN — do not code past these without Tahir

1. **Gates.** Keep Spec v2's G1–G6 (route-level, deliberate), reduce to the two hard
   rules in §0, or replace with obligations-and-deadlines? (Tahir leaning away from
   "gates and committees" as the frame.)
2. **Cost in PD.** `MODELING-GROUND-RULES.md` §0 forbids cost; PD schema + the NP
   engine both compute it (on placeholder prices). Must be settled: amend §0 to
   "O2S-scoped; PD owns cost as a decision criterion", or strip cost from PD.
   Sheikh Arshad's "what price?" already forced this.
3. **Evidence base into PD, or linked?** Rebuild the library per §6, or keep it in
   `E:\NP` and link. Rebuild is the bigger, righter job.
4. **Migration vs greenfield.** The current `pd/` build models Problem/Project/Route/
   gates/samples well but has no Question object, no gradeable Claim, no Request/
   customer-sample path. Decide: extend the existing schema, or rebuild the core.
5. **Pilot before software.** Run ONE live question in the open first (humic
   mechanism, or NP 11-44 NUE — both are ready) to prove the discipline changes
   behaviour, before spending on the build. The empty `NP 5-40` folders are the
   warning: a perfect empty container is what VAN produces when structure exists but
   nobody owes an answer.
