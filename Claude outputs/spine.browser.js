...
(async () => {
  ...
  /* ---- search, by clicking ---------------------------------------------
     The API suite covers what search returns. This covers the thing an API
     suite cannot see: that the box exists, that Enter works, that a result is
     clickable, and that adding it did not make a fifth nav item. */
  await page.goto(BASE + '/pd#problems', { waitUntil: 'domcontentloaded' });
  await waitForText(/Register one|Problems/);
  ok(await page.isVisible('#findq'), 'there is a search box on every screen');
  eq((await page.$$('.nav a')).length, 4, 'and adding it did not make a fifth nav item');
  await page.fill('#findq', 'humic');
  await page.press('#findq', 'Enter');
  await waitForText(/thing[s]? for/);
  const sm = await main();
  ok(/for .humic./.test(sm), 'pressing Enter searches for what was typed');
  ok(/Question|Problem|Bet|Run|Claim/.test(sm), 'and the results say what kind of thing each one is');
  ok((await page.$$('.hit')).length > 0, 'with at least one clickable result');
  await page.click('.hit');
  await waitForText(/back to problems/);
  ok(/P-0\d/.test(await main()) && /back to problems/.test(await main()), 'and clicking one opens the Problem it lives under');

  await page.goto(BASE + '/pd#search/' + encodeURIComponent('zzqqxx-nothing'), { waitUntil: 'domcontentloaded' });
  await waitForText(/Nothing found/);
  ok(/it was never written down/.test(await main()), 'an empty result explains itself rather than looking broken');
