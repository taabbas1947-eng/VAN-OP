...
/* the approval itself */
{
  ...
  vm.createContext(b); vm.runInContext(grab('approveCustomer'), b);
...
/* the job on Today */
{
  ...
  vm.createContext(b); vm.runInContext(grab('pendingCustomerItems'), b);
  const items = b.pendingCustomerItems();
  eq('one job per pending customer', items.length, 1);
  eq('addressed to the CFO', items[0].role, 'CFO');
  eq('with the approval as its action', items[0].act, "approveCustomer('A')");
