-- ===========================================================================
-- STATE-CHECK-PRODUCTION.sql — the same check as STATE-CHECK.sql, with every
-- table name written as jodilkah_vanop_db.<table> and DATABASE() replaced by
-- 'jodilkah_vanop_db'. Generated 25 September 2026.
--
-- WHY. On HostGator's phpMyAdmin the SQL tab is often stuck on
-- information_schema, and an unqualified table name then fails with
-- "Unknown table ... in information_schema" (seen on 11 Sept and again on
-- 25 Sept). Qualifying every name sidesteps the selected database entirely.
-- Read-only, like the original. Regenerate from STATE-CHECK.sql when that
-- file changes; do not edit this one by hand.
-- ===========================================================================
...
SELECT '003 (a)' AS migration, 'pd_field_history table' AS checks,
 ...
 WHERE TABLE_SCHEMA = 'jodilkah_vanop_db' AND TABLE_NAME = 'pd_field_history'
...
SELECT '003 (a)', 'append-only triggers on pd_field_history',
 ...
 WHERE TRIGGER_SCHEMA = 'jodilkah_vanop_db'
...
SELECT '003 (b)', 'pd_notices table',
  ...
  FROM information_schema.TABLES
 WHERE TABLE_SCHEMA = 'jodilkah_vanop_db' AND TABLE_NAME = 'pd_notices'

UNION ALL
SELECT '003 (c)', 'pd_observations.door_chosen column',
       IF(COUNT(*) > 0, 'APPLIED', 'NOT APPLIED'), CONCAT(COUNT(*), ' column')
  FROM information_schema.COLUMNS
 WHERE TABLE_SCHEMA = 'jodilkah_vanop_db' AND TABLE_NAME = 'pd_observations'
...
SELECT '004 (1a)', 'pd_materials assay columns made nullable',
  ...
  FROM information_schema.COLUMNS
 WHERE TABLE_SCHEMA = 'jodilkah_vanop_db' AND TABLE_NAME = 'pd_materials'
...
SELECT '005', 'UNIQUE (claim_number, version) on pd_claims',
 ...
 WHERE TABLE_SCHEMA = 'jodilkah_vanop_db' AND TABLE_NAME = 'pd_claims'
...
SELECT '006', 'pd_role ENUM carries the two new roles',
  ...
  FROM information_schema.COLUMNS
 WHERE TABLE_SCHEMA = 'jodilkah_vanop_db' AND TABLE_NAME = 'auth_users'
...
SELECT '007', 'the plant-wide context exists (id 90)',
  ...
  FROM jodilkah_vanop_db.pd_delivery_contexts
...
SELECT '008', 'pd_runs has the recipe box (recipe_text)',
       ...
       LEFT(MAX(COLUMN_TYPE), 200)
  FROM information_schema.COLUMNS
 WHERE TABLE_SCHEMA = 'jodilkah_vanop_db' AND TABLE_NAME = 'pd_runs'
...
SELECT 'grades loaded by 004' AS checks,
  ...
  FROM jodilkah_vanop_db.pd_materials
...
SELECT claim_number, version, COUNT(*) AS copies
  FROM jodilkah_vanop_db.pd_claims
...
SELECT * FROM jodilkah_vanop_db.auth_users WHERE pd_role IS NOT NULL ORDER BY pd_role;
...
SELECT 'pd_problems' AS t, COUNT(*) AS n FROM jodilkah_vanop_db.pd_problems
UNION ALL SELECT 'pd_questions',    COUNT(*) FROM jodilkah_vanop_db.pd_questions
UNION ALL SELECT 'pd_bets',         COUNT(*) FROM jodilkah_vanop_db.pd_bets
UNION ALL SELECT 'pd_runs',         COUNT(*) FROM jodilkah_vanop_db.pd_runs
UNION ALL SELECT 'pd_claims',       COUNT(*) FROM jodilkah_vanop_db.pd_claims
UNION ALL SELECT 'pd_challenges',   COUNT(*) FROM jodilkah_vanop_db.pd_challenges
UNION ALL SELECT 'pd_observations', COUNT(*) FROM jodilkah_vanop_db.pd_observations
UNION ALL SELECT 'pd_requests',     COUNT(*) FROM jodilkah_vanop_db.pd_requests
UNION ALL SELECT 'pd_constraints',  COUNT(*) FROM jodilkah_vanop_db.pd_constraints
UNION ALL SELECT 'pd_run_readings', COUNT(*) FROM jodilkah_vanop_db.pd_run_readings
UNION ALL SELECT 'pd_materials',    COUNT(*) FROM jodilkah_vanop_db.pd_materials;
